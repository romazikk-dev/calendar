    ...
    <div>
        ...
        <div class="range-slider flat"
            ...
            :style="{
                ...
                '--value-b': inputEnd,
                '--suffix': '%',
                '--text-value-a': inputStart,
                '--text-value-b': inputEnd,
            }">
                <input type="range"
                    :min="minDuration"
                    :max="maxDuration"
                    v-model="inputStart"
                    oninput="">
                <!-- <output>{{inputStartStrHoursAndMinutes}}</output> -->
                <input type="range"
                    :min="minDuration"
                    :max="maxDuration"
                    v-model="inputEnd"
                ...
                <div class="tickss">
                    <span class="min-tick float-left">{{minRangeStartStrHoursAndMinutes}}</span>
                    <span class="max-tick float-right">{{maxRangeEndStrHoursAndMinutes}}</span>
                </div>
        </div>
...
<script>
    export default {
        name: 'timeBarFillRange',
        mounted: function() {
            // console.log(JSON.parse(JSON.stringify(this.my)));
            // console.log(11111);
            // console.log(this.my);
        ...
        props: ['showOutput','outputTransparency','range','offset','mainColor'],
        data: function(){
            return {
                inputStart: 0,
                inputEnd: 100,
        ...
        computed: {
            minRangeStartStrHoursAndMinutes: function () {
                return calendarHelper.time.composeHourMinuteTimeFromMinutes(10);
            },
            maxRangeEndStrHoursAndMinutes: function () {
                return calendarHelper.time.composeHourMinuteTimeFromMinutes(180);
            },
            ...
            rightOffset: function () {
                if(typeof this.offset !== 'undefined' && this.offset !== null && !isNaN(this.offset))
                    return this.offset;
                return 0;
            },
        },
        methods: {
            setInputInitValue: function (range = null){
                // alert(2222);
                if(range === null)
                    range = this.range;
                    
                if(typeof range !== 'undefined' && typeof range.start !== 'undefined' &&
                typeof range.end !== 'undefined' && !isNaN(range.start) &&
                !isNaN(range.end)){
                    if(range.start > range.end){
                        this.inputStart = range.end;
        ...
        watch: {
            inputStart: function (val) {
                if(parseInt(val) > parseInt(this.inputEnd) - this.rightOffset){
                    this.inputStart = parseInt(this.inputEnd) - this.rightOffset;
                    return;
                }
                
                this.$refs.range_slider.style.setProperty('--value-a', val);
                this.$refs.range_slider.style.setProperty('--text-value-a', JSON.stringify(val));
                this.emitChange();
            },
            inputEnd: function (val) {
                if(parseInt(val) < parseInt(this.inputStart) + this.rightOffset){
                    this.inputEnd = parseInt(this.inputStart) + this.rightOffset;
                    return;
                }
                
                this.$refs.range_slider.style.setProperty('--value-b', val);
                this.$refs.range_slider.style.setProperty('--text-value-b', JSON.stringify(val));
                this.emitChange();
            },
    	}
    }
...
.range-slider {
  ...
  // --main-color: rgba(40,167,69, .7);
  --primary-color: var(--main-color, #328bf0);
